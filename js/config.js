            ...
            var _config = {
                ...
                touchStart: function(e) {
                    var event = e || window.event,
                        ...
                        handled_Image_Slide_Container ? _g(handled_Image_Slide_Container.querySelector('img') ? handled_Image_Slide_Container.querySelector('img').alt : '', '', '', '', '', 1) : 35,
                        imageParentNodeEnlargedWidth = handled_Image_Slide_Container && handled_Image_Slide_Container.querySelector('img') ? _g(handled_Image_Slide_Container.querySelector('img').alt, '', '', '', 1) : 90;
                    if (instance.c(event.touches[0].target, 'TABLE', 'slide') && instance.c(event.touches[0].target, 'TABLE', 'slide').parentNode.id == 'handledSlideContainer') {
                        event.preventDefault();
                        if (instance.slideLiCol && instance.slideLiCol.length) {
                            instance.slideLiCol[0].style.visibility = 'visible';
                            Array.prototype.splice.call(instance.slideLiCol, 0, 1);
                ...
                handler: function(e) {
                    var _e = e || event,
                        ...
                        subRoutine = function(eventType) {
                            var OpacityDivToBody = $('#OpacityDivToBody');
                            if (OpacityDivToBody.length && $('div[id$="Container"]').length !== 2) {
                                OpacityDivToBody.remove();
                            }
                    ...
                    } else if (_e.type === 'click' | 'onclick' | 'touchstart') {
                        executable = function() {
                            subRoutine(_e.type);
                        };
                    }
                ...
                slidesHandler: function(e) {
                    var _e = e || event,
                        ...
                        subRoutine = function(eventType) {
                            ...
                            _u(document, event.type, _config.scroller);
                            _u(window, eventType, _config.handler);
                            _u(window, 'keydown', _config.arrowPress);
                            _u(window, 'touchstart', _config.touchStart);
                        };
                        ...
                        if (!document.getElementById('handledImageContainer')) {
                            var _el = _config.documentCreateElement('div', 'handledImageContainer', [{
                                ...
                                _ex = _config.documentCreateElement('div', '', [{
                                    'background-image': "url('data/dealingWithDifficultPeople/images/close.png')"
                                }, {
                                    'position': 'absolute'
                                }, {
                            ...
                            if (!_g(targetEl.alt)['arrowPress']) {
                                ...
                                _e(window, 'touchstart', _config.touchStart);
                            }
        ...
        animatory: {
            ...
            appendStickyMenu: function() {
                if (!document.getElementById('stickingMenu')) {
					...
					if ($('#stickingMenu .breadcrumb').length < 1) {
                            $('#stickingMenu').prepend($('.columniser-text').has('.breadcrumb'));
                    }
                }
            },
            scrollTop: function() {
                ...
                var _interval,
                    ...
                    obverse = function() {
                        if (el && el.style.opacity > 0) {
                            ...
                            clearInterval(_interval);
                            config.animatory.activeState = false;
                            $(window).trigger('activeStateChange');
                            return true;
                        }
                    };
            ...
            _windowSrollY: 0,
            handler: function(e) {
                if (!config.closest(e.target, 'DIV', 'breadcrumb')) {
                    var _animatory = {
                        runOnce: false,
                    ...
                    if (_animatory.upwards()) {
                        ...
                        $(window).on('activeStateChange', function() {
                            if ($('div.header .breadcrumb').length < 1) {
                                $('div.header').append($('.columniser-text').has('.breadcrumb'));
                            }
                        });
                    }
